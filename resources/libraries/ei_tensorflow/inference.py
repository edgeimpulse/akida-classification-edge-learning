...
def process_output_yolov5(output_details, interpreter, img_shape, version, minimum_confidence_rating=None):
...
def run_yolov5_inference(interpreter: Interpreter, item: np.ndarray, specific_input_shape: 'list[int]',
    ...
    raw_scores = process_output_yolov5(output_details, interpreter, (width, height),
...
def run_yolox_inference(interpreter: Interpreter, item: np.ndarray, specific_input_shape: 'list[int]',
    ...
    raw_scores = process_output_yolox(output_details, interpreter, img_size=width, minimum_confidence_rating=minimum_confidence_rating)
    return compute_performance_object_detection(raw_scores, input_width_height, y_data, num_classes)
...
def process_output_yolox(output_details, interpreter, img_size, minimum_confidence_rating=None):
    """Transforms an output tensor into a Python list for object detection
    models.
    Args:
        output_details: The result of calling interpreter.get_output_details()
