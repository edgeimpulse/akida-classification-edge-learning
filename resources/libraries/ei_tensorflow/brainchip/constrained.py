...
def build_model(input_shape: tuple, alpha: float,
    ...
    pretrained_weights = './transfer-learning-weights/akidanet/akidanet_imagenet_224_alpha_50_iq8_wq4_aq4.h5'
    ...
    a_model_part_head = Conv2D(filters=32, kernel_size=1, strides=1, padding='same',
                activation='linear', name='head')(a_cut_point.output)
    a_model_part = ReLU(max_value=1, negative_slope=0.0, threshold=0.0, name='headact')(a_model_part_head)
    a_logits = Conv2D(filters=num_classes, kernel_size=1, strides=1, padding='same',
                    activation=None, name='logits')(a_model_part)
