...
def train_model(train_dataset, validation_dataset, input_length, callbacks,
    ...
    # model architecture
    model = Sequential()
    model.add(Conv2D(32, kernel_size=3, kernel_constraint=tf.keras.constraints.MaxNorm(1), padding='same'))
    model.add(ReLU())
    model.add(MaxPooling2D(pool_size=2, strides=2, padding='same'))
    model.add(Conv2D(16, kernel_size=3, kernel_constraint=tf.keras.constraints.MaxNorm(1), padding='same'))
    model.add(ReLU())
    model.add(MaxPooling2D(pool_size=2, strides=2, padding='same'))
    model.add(Flatten())
